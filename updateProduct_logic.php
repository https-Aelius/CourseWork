...
//ensuring the itemID came from my form. 
$itemID = $_POST['itemID'];

echo'itemID is: ' . $itemID . '<br>';

//making the image fields null first before updating them with new filenames
if ($_FILES['fileToUpload1']['error'] && ['fileToUpload2']['error']=== UPLOAD_ERR_OK) {
    $stmt = $conn->prepare('UPDATE Products 
        SET productImage = NULL, dimensionImage = NULL
        WHERE itemID = :itemID');
    $stmt->bindParam(':itemID', $itemID);
    $stmt->execute();
    echo '<br>finished making images null<br>';
}
...
echo'finished update statement<br>';
//for file validation

if ($_FILES['fileToUpload1']['error'] === UPLOAD_ERR_OK) {
    $files = glob("/Applications/XAMPP/xamppfiles/htdocs/Coursework4School/Images/{$itemID}_product.*");
...
if ($_FILES['fileToUpload2']['error'] === UPLOAD_ERR_OK) {
    ...
    $files = glob("/Applications/XAMPP/xamppfiles/htdocs/Coursework4School/Images/{$itemID}_dimension.*");
...
$filename1= $itemID . '_product';
$filename2= $itemID . '_dimension';
...
if ($_FILES['fileToUpload1']['size'] > 5000000 || $_FILES['fileToUpload2']['size'] > 5000000) {
    ...
    header('Location: updateStockPage.php');
    exit();
...
if (move_uploaded_file($_FILES['fileToUpload1']['tmp_name'], $target_file1) && move_uploaded_file($_FILES['fileToUpload2']['tmp_name'], $target_file2)) {
    ...
    header('Location: updateStockPage.php');
    //we can ignore this since the when files don't get uploaded its because the previous files are still in database. 
    $_SESSION['Message5'] = "<p style='color:green; font-size:10px;'>Product added successfully! File has been uploaded.</p>";
    exit();
